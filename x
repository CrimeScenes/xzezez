...
local function isWithinBoxFOV(position)
    ...
    local fovHeight = getgenv().Forbidden.TriggerBot.Settings.BoxFOVSize.Height * 120  -- Increased for better range
    local fovWidth = getgenv().Forbidden.TriggerBot.Settings.BoxFOVSize.Width * 120
...
local function isInLineOfSight(targetPart)
    ...
    -- Check if there's any obstruction between the origin and target part
...
local function getPredictedPosition(part)
    if part then
        ...
        local timeToPredict = getgenv().Forbidden.TriggerBot.Settings.Preds.TimeToPredict or 0.15  -- Slightly increased for better accuracy
...
local function adjustAimForTarget(targetPosition)
    ...
    if onScreen then
        ...
        local smoothFactor = 0.85  -- Added smoother aiming factor for better control
        VirtualInputManager:SendMouseMoveEvent(centerX + offsetX * smoothFactor, centerY + offsetY * smoothFactor, game)
    end
end
                    ...
                    if part then
                        ...
                        if predictedPosition and isWithinBoxFOV(predictedPosition) and isInLineOfSight(part) and os.clock() - lastClickTime >= getgenv().Forbidden.TriggerBot.Settings.Cooldown then
                            ...
                            if tool and tool:IsA("Tool") then
                                ...
                                return  -- Stop after firing on a valid part
                            end
                        end
                    end
                end
...
local groupIds = { 33986332, 234567, 345678 } -- Replace with your group IDs
local kickOnGroupJoin = getgenv().Forbidden.ModCheck.KickIfModJoined -- Assuming this is defined elsewhere

-- Function to check for group joins
game:GetService("Players").PlayerAdded:Connect(function(player)
    if getgenv().Forbidden.ModCheck.Enabled then
        for _, groupId in ipairs(groupIds) do
            local rank = player:GetRankInGroup(groupId)
            if rank > 0 then  -- Check if the player is in the group (rank > 0 means they are a member)
                -- User is a member of the specific group
                sendNotification("Mod Alert", player.Name .. " (a mod) has joined your game!")

                if kickOnGroupJoin then
                    -- Kick the player with a specific message
                    game.Players.LocalPlayer:Kick(player.Name .. " (a mod) has joined, you have been kicked.")
                end
                break -- Exit the loop after notifying or kicking the player
            end
        end
    end
end)
...
-- Update targeting logic on key press
...
local Games = {
    ...
    [128702711425162] = {Name = "OG Da Hood", Argument = "UpdateMousePos", Remote = "MainEvent", BodyEffects = "K.O",},
    ...
    [16871610060] = {Name = "Da Comp", Argument = "UpdateMousePos", Remote = "MainEvent", BodyEffects = "K.O",},
}
...
Run_Service.RenderStepped:Connect(function()
    if getgenv().Forbidden.Silent.Normal.Enabled and IsTargeting then  -- Only work when targeting is enabled
        ...
        if Target and Target.Character then
            ...
            if closestPart and closestPoint then
                -- Check if the target is specifically the one that should be aimed at
                if TargetPlayer and TargetPlayer == Target then
...
-- Make sure that the TargetPlayer is set correctly when camlocked
RunService.RenderStepped:Connect(function()
    if IsTargeting then
        TargetPlayer = ClosestPlrFromMouse()  -- Continuously update the target while camlocked
    end
end)
...
local function onCharacterAdded(character)
    character.ChildAdded:Connect(HookTool) -- Hook tool when a new tool is added
end

-- Connect the CharacterAdded event
Local_Player.CharacterAdded:Connect(onCharacterAdded)
if Local_Player.Character then
    onCharacterAdded(Local_Player.Character) -- Call on existing character
end

-- Function to check for no ground shots
if getgenv().Forbidden.Adjustment.Checks.NoGroundShots == true then
    ...
    -- Connect the function to all players
    game.Players.PlayerAdded:Connect(function(Plr)
        Plr.CharacterAdded:Connect(function()
            CheckNoGroundShots(Plr)
        end)
    end)

    -- Check for the local player character if already present
    if Local_Player.Character then
        CheckNoGroundShots(Local_Player)
    end
end
